...
// Tab types
type PrimaryTab = 'dashboard' | 'rooms' | 'allocations' | 'fees' | 'mess' | 'attendance'
        ...
        {statCards.map((stat) => (
          <Card key={stat.title}>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <stat.icon className="h-4 w-4" style={{ color: 'var(--color-module-hostel)' }} />
            </CardHeader>
            <CardContent>
...
function RoomsTab() {
  ...
  const getStatusColor = (status: string): React.CSSProperties => {
    switch (status) {
      case 'available':
        return { backgroundColor: 'var(--color-module-hostel-light)', color: 'var(--color-module-hostel)' }
      case 'full':
        return { backgroundColor: 'var(--color-module-hostel-light)', color: 'var(--color-module-hostel)' }
      case 'maintenance':
            ...
            <Card key={room.id}>
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge style={getStatusColor(room.status)}>
              ...
              <TableBody>
                {allocations.map((allocation) => (
                  <TableRow key={allocation.id}>
                    ...
                    <TableCell>
                      <Badge style={getStatusColor(allocation.status)}>
...
function FeesTab() {
  ...
  const getStatusColor = (status: string): React.CSSProperties => {
    switch (status) {
      ...
      case 'overdue':
        return { backgroundColor: 'var(--color-status-overdue-light)', color: 'var(--color-status-overdue)' }
      default:
        return { backgroundColor: 'var(--color-status-inactive-light)', color: 'var(--color-status-inactive)' }
    }
  }
...
// ============================================
// Main HostelMainPage Component
// ============================================
export function HostelMainPage() {
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Manage hostels, rooms, allocations, fees, mess, and attendance"
