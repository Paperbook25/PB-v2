...
// Tab types
type PrimaryTab = 'dashboard' | 'assets' | 'stock' | 'purchase-orders' | 'vendors'
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {statCards.map((stat) => (
          <Card
            key={stat.title}
            className={stat.alert ? 'border-yellow-500' : ''}
            style={stat.primary && !stat.alert ? { borderColor: 'var(--color-module-inventory)' } : undefined}
          >
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium">{stat.title}</CardTitle>
              <stat.icon
                className={`h-4 w-4 ${stat.alert ? 'text-yellow-500' : !stat.primary ? 'text-muted-foreground' : ''}`}
                style={stat.primary && !stat.alert ? { color: 'var(--color-module-inventory)' } : undefined}
              />
            </CardHeader>
            <CardContent>
              <div
                className={`text-2xl font-bold ${stat.alert ? 'text-yellow-600' : ''}`}
                style={stat.primary && !stat.alert ? { color: 'var(--color-module-inventory)' } : undefined}
              >
                {stat.value}
              </div>
...
function AssetsTab() {
  ...
  const getStatusColor = (status: string): React.CSSProperties => {
    switch (status) {
      case 'active':
        return { backgroundColor: 'var(--color-module-inventory-light)', color: 'var(--color-module-inventory)' }
      case 'maintenance':
        return { backgroundColor: 'var(--color-status-maintenance-light)', color: 'var(--color-status-maintenance)' }
      case 'disposed':
        return { backgroundColor: 'var(--color-status-inactive-light)', color: 'var(--color-status-inactive)' }
      case 'lost':
        return { backgroundColor: 'var(--color-status-error-light)', color: 'var(--color-status-error)' }
      default:
        return { backgroundColor: 'var(--color-status-inactive-light)', color: 'var(--color-status-inactive)' }
    }
  }

  const getConditionColor = (condition: string): React.CSSProperties => {
    switch (condition) {
      case 'new':
        return { color: 'var(--color-module-inventory)' }
      case 'good':
        return { color: 'var(--color-module-inventory)' }
      case 'fair':
      ...
      default:
        return { color: 'var(--color-status-inactive)' }
    }
  }
              ...
              <TableBody>
                {assets.slice(0, 20).map((asset) => (
                  <TableRow key={asset.id}>
                    ...
                    <TableCell>
                      <span style={getConditionColor(asset.condition)}>
                        {ASSET_CONDITION_LABELS[asset.condition]}
                      </span>
                    </TableCell>
                    <TableCell>
                      <Badge style={getStatusColor(asset.status)}>
  ...
  const getStatusColor = (status: string): React.CSSProperties => {
    switch (status) {
      ...
      case 'pending_approval':
        return { backgroundColor: 'var(--color-status-pending-light)', color: 'var(--color-status-pending)' }
      case 'approved':
        return { backgroundColor: 'var(--color-module-inventory-light)', color: 'var(--color-module-inventory)' }
      case 'ordered':
        return { backgroundColor: 'var(--color-module-integrations-light)', color: 'var(--color-module-integrations)' }
      case 'received':
        return { backgroundColor: 'var(--color-status-success-light)', color: 'var(--color-status-success)' }
      case 'cancelled':
        return { backgroundColor: 'var(--color-status-error-light)', color: 'var(--color-status-error)' }
      default:
        return { backgroundColor: 'var(--color-status-inactive-light)', color: 'var(--color-status-inactive)' }
                ...
                {purchaseOrders.map((po) => {
                  ...
                  return (
                    <TableRow key={po.id}>
                      ...
                      <TableCell>
                        <Badge style={getStatusColor(po.status)}>
                          {PO_STATUS_LABELS[po.status]}
                        </Badge>
                      </TableCell>
                      <TableCell>
...
// ============================================
// Main InventoryMainPage Component
// ============================================
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Manage assets, stock, purchase orders, and vendors"
