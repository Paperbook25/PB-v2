...
export function StudentDetailPage() {
  ...
  const { data: marksResult, isLoading: marksLoading } = useStudentMarks(id ?? '', undefined, { enabled: !!id && activeTab === 'academic' })
  const { data: attendanceResult, isLoading: attendanceLoading } = useStudentAttendance(id ?? '', undefined, { enabled: !!id && activeTab === 'attendance' })
  const { data: feesResult, isLoading: feesLoading } = useStudentFeesById(id ?? '', { enabled: !!id && activeTab === 'fees' })
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <InfoRow icon={User} label="Gender" value={student.gender ? student.gender.charAt(0).toUpperCase() + student.gender.slice(1) : '—'} />
                <InfoRow
                  icon={MapPin}
                  label="Address"
                  value={student.address ? `${student.address.street || ''}, ${student.address.city || ''}, ${student.address.state || ''} - ${student.address.pincode || ''}` : '—'}
                />
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent>
              {marksLoading ? (
                ...
                const marks = Array.isArray(marksResult?.data) ? marksResult.data : []
          ...
          <Card>
            ...
            <CardContent>
              {attendanceLoading ? (
                ...
                if (!att?.summary) return <p className="text-center py-8 text-muted-foreground">No attendance data available</p>
                return (
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--color-module-attendance-light)' }}>
                      <p className="text-2xl font-bold" style={{ color: 'var(--color-module-attendance)' }}>{att.summary.attendancePercentage ?? 0}%</p>
                      <p className="text-sm text-muted-foreground">Overall Attendance</p>
                    </div>
                    <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--color-module-communication-light)' }}>
                      <p className="text-2xl font-bold" style={{ color: 'var(--color-module-communication)' }}>{att.summary.presentDays ?? 0}</p>
                      <p className="text-sm text-muted-foreground">Days Present</p>
                    </div>
                    <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--color-module-exams-light)' }}>
                      <p className="text-2xl font-bold" style={{ color: 'var(--color-module-exams)' }}>{att.summary.absentDays ?? 0}</p>
          ...
          <Card>
            ...
            <CardContent>
              {feesLoading ? (
                ...
                const fees = Array.isArray(feesResult?.data) ? feesResult.data : []
