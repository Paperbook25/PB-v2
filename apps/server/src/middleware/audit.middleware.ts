...
const SENSITIVE_FIELDS = ['password', 'adminPassword', 'passwordHash', 'refreshToken', 'accessToken', 'secret']
      ...
      if (req.user) {
        prisma.auditLog.create({
          data: {
            ...
            changes: req.method === 'PUT' || req.method === 'PATCH' ? sanitizeBody(req.body) : undefined,
          },
        }).catch((err) => {
