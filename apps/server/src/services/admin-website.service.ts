...
Write the blog post now:`
  ...
  if (env.LLM_PROVIDER === 'gemini' && env.GEMINI_API_KEY) {
    content = await callGemini(ollamaPrompt)
    if (content) {
      console.log(`[Blog Writer] Generated ${content.length} chars via Gemini 1.5 Flash`)
    }
  } else if (env.LLM_PROVIDER === 'ollama' || !env.OPENAI_API_KEY) {
