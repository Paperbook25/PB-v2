...
export function SignupPage() {
  ...
  const handleNext = async () => {
    ...
    if (step === 2) {
      ...
      setError('')
      try {
        const res = await fetch(`${API_URL}/api/public/register-school`, {
          method: 'POST',
          ...
          {step === 3 && result && (
            <div className="text-center py-4">
              ...
              <p className="text-xs text-gray-400 mb-6">
                Your school URL: <code className="bg-gray-100 px-1.5 py-0.5 rounded">{result.slug}.{APP_DOMAIN}</code>
              </p>
              <button
                onClick={() => {
                  window.location.href = `https://${result.slug}.${APP_DOMAIN}/login`
                }}
