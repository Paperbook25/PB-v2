...
// Tab types
type PrimaryTab = 'list' | 'attendance' | 'leave' | 'payroll' | 'timetable' | 'substitutions'
...
// ============================================
function AttendanceTab({ subTab, onSubTabChange }: { subTab: AttendanceSubTab; onSubTabChange: (tab: AttendanceSubTab) => void }) {
  const { toast } = useToast()
...
// Leave Tab Component
// ============================================
function LeaveTab({ subTab, onSubTabChange }: { subTab: LeaveSubTab; onSubTabChange: (tab: LeaveSubTab) => void }) {
  const { toast } = useToast()
...
// ============================================
function PayrollTab({ subTab, onSubTabChange }: { subTab: PayrollSubTab; onSubTabChange: (tab: PayrollSubTab) => void }) {
  const { toast } = useToast()
  const [search, setSearch] = useState('')
...
export function StaffPage() {
  ...
  // Primary tab
  const activeTab = (searchParams.get('tab') as PrimaryTab) || 'list'

  // Sub-tabs with defaults
  const attendanceSubTab = (searchParams.get('subtab') as AttendanceSubTab) || 'mark'
  const leaveSubTab = (searchParams.get('subtab') as LeaveSubTab) || 'pending'
  const payrollSubTab = (searchParams.get('subtab') as PayrollSubTab) || 'process'
  const timetableSubTab = (searchParams.get('subtab') as TimetableSubTab) || 'class'
        ...
        <div className="mt-6">
          ...
          <TabsContent value="attendance" className="mt-0">
            <AttendanceTab
              ...
              onSubTabChange={(tab) => handleSubTabChange('attendance', tab)}
            />
          </TabsContent>
          ...
          <TabsContent value="leave" className="mt-0">
            <LeaveTab
              subTab={leaveSubTab}
              onSubTabChange={(tab) => handleSubTabChange('leave', tab)}
            />
          </TabsContent>
          ...
          <TabsContent value="payroll" className="mt-0">
            <PayrollTab
              ...
              onSubTabChange={(tab) => handleSubTabChange('payroll', tab)}
            />
          </TabsContent>
          ...
          <TabsContent value="timetable" className="mt-0">
            <TimetableTab
              ...
              onSubTabChange={(tab) => handleSubTabChange('timetable', tab)}
            />
          </TabsContent>
